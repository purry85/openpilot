...
static void update_state(UIState *s) {
  ...
  if (sm.updated("modelV2") && s->vg) {
    ...
    auto lead_state = sm["modelV2"].getModelV2().getLeads()[0];
    s->scene.lead_v_rel = lead_state.getXyva()[2];
    s->scene.lead_d_rel = lead_state.getXyva()[0];
	  if (lead_state.getProb() > 0.5) {
